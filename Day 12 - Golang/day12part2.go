...
func main() {
	...
	for _, v := range coordinates {
		if v.val == "S" || v.val == "a" {
			startVals = append(startVals, coord{v.x, v.y, "a", nil})
		} else if v.val == "E" {
	...
	possiblePathLens := []int{}
	for _, v := range startVals {
		path := pathSearch(v, goalVal)
		if path == nil {
			continue
		}
		possiblePathLens = append(possiblePathLens, len(path))
	}

	sort.Sort(sort.IntSlice(possiblePathLens))

	fmt.Println(possiblePathLens[0])
}
